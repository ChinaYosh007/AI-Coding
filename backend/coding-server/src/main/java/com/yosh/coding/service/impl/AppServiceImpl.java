...
public class AppServiceImpl extends ServiceImpl<AppMapper, App>  implements AppService{
    ...
    public CodeGenTypeEnum generateRoute(String prompt) {
        ...
        CodeGenTypeResult codeGenTypeResult = aiCodeGenTypeRoutingService.routeCodeGenType(prompt);

        if (codeGenTypeResult == null || StrUtil.isBlank(codeGenTypeResult.getCodeGenType())) {
            throw new BusinessException(ErrorCode.OPERATION_ERROR, "AI路由分析失败");
        }
        try {
            return CodeGenTypeEnum.valueOf(codeGenTypeResult.getCodeGenType());
        } catch (IllegalArgumentException e) {
            log.error("AI返回的代码生成类型无效: {}", codeGenTypeResult.getCodeGenType(), e);
            throw new BusinessException(ErrorCode.OPERATION_ERROR, "AI返回的代码生成类型无效: " + codeGenTypeResult.getCodeGenType());
        }
    }
    ...
    public String developApp(Long appId, LoginUserVO loginUser,Long version){
        ...
        if(app.getCodeGenType().equals(CodeGenTypeEnum.VUE_PROJECT.getValue())){
            ...
            ThrowUtils.throwIf(!dir.exists() || !dir.isDirectory(),ErrorCode.NOT_FOUND_ERROR,"please build app of your");
            sourceDirFile = dir;
            log.info("builder success vue -------->_<-------------------");
        }
